...
2. **Spawn a sub-agent** to find unimplemented doc sections:
   ...
   For each file, check its `implementation:` entries (values are single-quoted markdown links `'[text](/path)'` — extract the path portion from each):
...
6. **For each selected doc**, one at a time:
   ...
   - After implementation: update the doc's `implementation:` frontmatter with the new file paths using single-quoted markdown links with a leading `/`, e.g. `'[filename.ts](/path/to/file.ts)'`
