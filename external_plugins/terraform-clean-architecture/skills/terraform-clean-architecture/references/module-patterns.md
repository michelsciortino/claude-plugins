...
> **Part of:** [terraform-clean-architecture](../SKILL.md)
...
6. [Variable Best Practices](#variable-best-practices)
7. [Output Best Practices](#output-best-practices)
8. [Common Patterns](#common-patterns)
9. [Anti-patterns to Avoid](#anti-patterns-to-avoid)
10. [Module Naming Conventions](#module-naming-conventions)
11. [Testing Your Modules](#testing-your-modules)
12. [Testing Philosophy & Patterns](#testing-philosophy--patterns)
...
Use `use_lockfile = true` for native S3 state locking. DynamoDB-based locking is deprecated.
...
### Backend Config File Naming

Use `*.backendname.tfbackend` naming convention for partial backend configurations:
...
- Backend blocks cannot reference variables, locals, or data sources
- A configuration cannot have both a `cloud` block and a `backend` block
- Never hardcode credentials in backend config — use environment variables or `-backend-config`

---

## Repository Strategy

### One Repo Per Reusable Module

Store each reusable module in its own repository for independent versioning and clear ownership.

```
# ✅ GOOD - Independent repos
github.com/org/terraform-aws-vpc         # v2.3.1
github.com/org/terraform-aws-rds         # v1.8.0
github.com/org/terraform-aws-ecs         # v3.1.2

# ❌ BAD - Monorepo for reusable modules
github.com/org/terraform-modules/
  vpc/
  rds/
  ecs/   # Can't version independently
```

### Environment Separation

Use separate directories per environment, each with its own state file and `.tfvars`:
...
Avoid Terraform CLI workspaces for environment separation — use separate directories instead.
...
# Exclude .tfvars files (may contain sensitive data)
*.tfvars
*.tfvars.json
...
# It ensures reproducible provider installations
```
...
### DON'T: Wrap a Single Resource in a Module

If the module name matches the resource type name, it's not creating a useful abstraction.

```hcl
# ❌ BAD - Module adds no value
module "s3_bucket" {
  source = "./modules/s3_bucket"
  # Just wraps aws_s3_bucket with no added logic
}

# ✅ GOOD - Module adds real abstraction
module "encrypted_data_store" {
  source = "./modules/encrypted-data-store"
  # Combines S3 + encryption + logging + lifecycle rules
}
```

### DON'T: Nest Modules More Than 1-2 Levels Deep

Keep the module tree flat. Deep nesting makes debugging, state management, and refactoring painful.

### DON'T: Configure Providers in Shared Modules

Provider configuration (region, credentials, endpoints) belongs only in the root module. See the Provider Configuration Rule in the main skill file.
