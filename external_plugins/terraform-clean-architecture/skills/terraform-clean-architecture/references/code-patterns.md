...
variable "environment" {
  ...
  type        = string
  default     = "dev"
  sensitive   = false  # Explicitly not sensitive — shown here to illustrate ordering
  nullable    = false
...
resource "aws_instance" "app" {
  ...
  tags = {
    ...
    InstanceCount = each.value.instance_count  # Used by ASG, not Terraform count
  }
}
```
...
**Use `ephemeral = true` for short-lived credentials that should never be stored in state:**

```hcl
# ✅ GOOD - Short-lived token never stored in state
variable "temporary_token" {
  description = "Short-lived authentication token for deployment"
  type        = string
  sensitive   = true
  ephemeral   = true  # Never stored in state
}

# Use ephemeral variables for:
# - OIDC tokens
# - Temporary AWS credentials
# - Short-lived API keys
# - Any secret that should not persist in state
```

**Note:** Ephemeral variables are only available during the Terraform operation. They cannot be used in resources that store their value in state.
